...
View(county_voters_combined) #problem: total and TOTAL are not the same, need to go back to data and rename the last dataset total
```
...
**Answer**More voters over time. Increased 18,000 democrats and some more republicans. People with different political views are moving to Frederick. 
...
#population data allows us to do per capita stuff 
```
...
maryland_population |> left_join(county_voters_2022, join_by("COUNTY")) |> nrow() #2022 is more recent, 2020 took a long time to complete, starts with one dataframe, the one with more records go first, directional (wants to see every row even if there is not a match, inner_join only includes data that matches)
```
...
**Answer**Includes the population for a variety of races and age in Maryland
...
```{r}
maryland_population_with_voters_and_demographics <- maryland_population_with_voters |> left_join(maryland_demographics, join_by(COUNTY == NAME)) #new dataframe
```
...
**Answer**Prince George's County, 61% 18+ and 77% of Democrats
...
**Answer**Howard County, 19% 18+ and 53% Democrats. Asian Americans elect peolpe who represent them and people with the same race tend to stick together. 
...
**Answer**a number, but they should not be a number data type because you're never going to use zip codes in a math equation. need to use mutate to change to a character 
...
**Answer**Now it's a character datatype
