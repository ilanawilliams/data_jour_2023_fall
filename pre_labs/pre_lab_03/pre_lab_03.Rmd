...
**Answer** The warning says "one or more parsing issues, call 'problems()' on your data frame for details. The main problem is that guess_max() is set to 10. When we look at column four in the data, R does not include the zero for the NJ zip codes. We don't know if we're missing the leading zero or another number.  
...
**Answer**The zip codes change to character and the zeros are included. R looks at all the data instead of the first 10 rows. 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** "Vendor Name" is a character field, "Amount" is the number field and "Data" is a character field. Spaces in column name is not good so you have to add "back-ticks." 
...
**Answer**Over 500 zip codes have less than five characters. The data contains postal codes and zip codes from the U.S. and other countries, which make the zip code column messed up and difficult to use. 
...
**Answer**There's one category in 2010 that is not a grant or loan and the same problem in 2011 and futher down. There are no loans in 2009 (might not have the data). There's a contract in 2017 that's not a grant or loan. Also there's a category called L?? All these show gaps.  
...
**Answer**The lowest amount is a $60 grant to the Dept. of Natural Resources in 2009, which is not a lot. Can throw off the average because most amounts are larger. 
...
**Answer** clean_names is a function only in janitor, rename changes the name of the column with a new name (that has to come first) and the mutate function changes the columns to upper-case. Each name is in the same mutate function. Shows consistency  
...
**Answer**What likely happened was the same person receiving the grant got it multiple times a year because the date is the same. There is an infinite amount of duplicates??
...
**Answer** str_sub took the first five characters of the zip code and made a new column at the end. 
...
MD tourism board --> filter(source) --> FROM 
which zip code --> group_by(zip5)
most amount of money --> summarise()

**Answer**zip code 21202 got the most money. The zip code is Baltimore. I think that makes sense because Balitmore has the harbor and tourist attractions that many people flock to. 

```{r}
cleaned_md_grants_loans |>
  filter(source == "MARYLAND TOURISM BOARD") |> 
  group_by(zip5) |>
  summarise(money = sum(amount))
