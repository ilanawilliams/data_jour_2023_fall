...
**Answer**shows a list of zip codes and geometry functions
...
**Answer**Shows all the needed information that will be used to make the map. Zipcode2 is an identifier to understand which zip code comes from which state (024 is Maryland)
...
**Task** Run the following code. Describe the output in the space below. **Answer**The map shows the zip codes using the longitude and latitude to know where the zip codes are located. Translates the address to coordinates. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**tigris is installed. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**retrieves data from the entire U.S. Includes things like the GEOID, land and water areas and funky geometry things. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**Changes the information in the dataframe.  
...
**Task** Run the following code. Describe the output in the space below. **Answer**The map shows Maryland counties. This map does not include the Bay, which is fine because counties split the Bay.
...
**Task** Run the following code. Describe the output in the space below. **Answer**Layered the two maps to show the counties and zip codes (and where the Bay should be).The zip code boundaries end at the shore of the Potomac River. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**Data only goes up to March. Cleaned data with the zip code intent to foreclose in March BUT does not only show MD zip codes. Also, there is not geometry file so we need to connect it to data that does.
...
**Task** Run the following code. Describe the output in the space below. **Answer**We want all the MD zip codes and if there is a match with the foreclosure zip code and all the MD zip codes even if there is not a foreclosure notice. Filters out non-MD zips. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**Notices in March is a fill value. Differentiate the zip code by color. Lighter color is the notices with intent to foreclose
...
**Answer**Loaded libraries and I already have the API Key 
...
**Task** Run the following code. Describe the output in the space below. **Answer**No geometry function
...
**Task** Run the following code. Describe the output in the space below. **Answer**Problem is reduced but there is still a problem. Can join the county but needs some work.
...
**Task** Run the following code. Describe the output in the space below. **Answer**Loaded the county and population in the dataframe
...
**Task** Run the following code. Describe the output in the space below. **Answer**Remove trailing white space on both sides of county columns. All you get is county, population and geometry and can now join y county
...
**Task** Run the following code. Describe the output in the space below. **Answer**The dataframes join and shows the columns we needed
...
**Task** Run the following code. Describe the output in the space below. **Answer**Redo the join but adds the rate per 1000. Charles County has the highest rate then PG County and Baltimore City. Low foreclosure shows a function of wealth
...
**Task** Run the following code. Describe the output in the space below. **Answer**Finds the center of the counties and include the names. Color is hard to tell what the counties are. Is a reporting map (Charles County)

```{r}

county_centroids <- st_centroid(md_counties) #finding geo center of counties to lablel
...
**Task** Run the following code. Describe the output in the space below. **Answer**Changed the color and got rid of the labels. Highlights Charles County but hard to show the differences between the other counties. Scale_fill creates more distinction between fill range. 
...
**Answer**Shows a better gradient. trans="log" changes the scale. Shows the best way to understand the data ("how might the groups break down?")
