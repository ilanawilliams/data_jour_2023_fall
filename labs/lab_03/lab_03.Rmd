...
pg_schools <- cleaned_payments |> #to make a new dataframe, put new name in the front, to check look in environment
  ...
  )#there were more variations, but i wouldn't have known, did not include community college because we're looking at K-12 vs higher education
```
...
group_by(vendor_name, vendor_zip)|>
  summarise(total_amount=sum(amount)) #I don't understand the zip code part of the question; I think I'm missing something??
...
summarise(total_amount=sum(amount)) #I think I might be missing something??
