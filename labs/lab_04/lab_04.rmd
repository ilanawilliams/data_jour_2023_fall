...
media_expenses <- maryland_expenses |>
 filter(
   str_detect(expense_category,"Media") #how do you make payee NA? 
  )

write_csv(x=media_expenses, "data/media_expenses.csv")
media_is_cool <- read_csv("data/media_is_cool.csv")

total <- media_is_cool|>
  filter(payee_clean=='Facebook')|>
  group_by(committee_name)|>
  summarise(total_amount=sum(amount)) |>
  arrange(desc(total_amount))#look at the environment for new data
...
**A2.**The top committee is for Jain for governor, then Franchot and Franchot again  (For lack of time, I only looked up the top three committees). When I looked up Jain the ads were removed because it didn't follow the policy (however, I don't think this is right)
...
what function/tool do i use to only give me records in my dataset related to a value. value=winner of demo party

**A3.**Wes Moore won the primary.
...
**A4.**I don't totally know how to answer this. 

```{r}
maryland_expenses <- read_csv("data/maryland_expenses.csv")|>
group_by(committee_name)|>
summarise(online_ads=sum(amount)) #what am I missing? I think I need to filter something. 
```
...
**A5**Facebook has been utilized for many candidates. Would be interesting to know how much success they got from running ads on Facebook. 
